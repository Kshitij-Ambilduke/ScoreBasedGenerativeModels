...
def plot_sigma_over_schedulers(sigma_1, sigma_L, L, schedule_types, use_log=False):
    ...
    for schedule_type in schedule_types:
        ...
        plt.plot(torch.arange(1, L+1), sigmas, label=schedule_type, marker='o')
    plt.title("Discrete variance scheduling methods")
    plt.xlabel("Level i")
    plt.ylabel("Noise level σ_i" + (" (log scale)" if use_log else ""))
    plt.legend()
    if use_log:
...
if __name__ == "__main__":
    ...
    L=10
    # schedule_types = ['geometric', 'linear', 'quadratic', 'sqrt', 'sigmoid']
    schedule_types = ['geometric', 'linear', 'sqrt']
    plot_sigma_over_schedulers(sigma_1, sigma_L, L, schedule_types)
