...
def parse_args():
    p = argparse.ArgumentParser(description="Run PCA-based sampling with CondRefineNetDilated")
    p.add_argument("--batch-size", "-b", type=int, default=8, help="Batch size for sampling")
    p.add_argument("--model-path", "-m", type=str, default="author_code/PCA/SAVED_MODEL_4_fmnist.pt", help="Path to model .pt file")
    p.add_argument("--pca-path", type=str, default="author_code/PCA/PCA_64_012.joblib", help="Path to PCA joblib file")
...
def main():
    ...
    # initialize in PCA space (64 dim -> 8x8)
    x_t = torch.rand(batch_size, 1, 8, 8, device=device)
    ...
    # load PCA model and inverse transform each snapshot (per-batch, per-level)
    PCA_model = joblib.load(args.pca_path)

    # prepare images for plotting: list of length L with tensors shape (batch_size, 28, 28)
    images_per_level = []
    for level_tensor in batch_snapshots:
        # level_tensor: (batch_size, 1, 8, 8)
        imgs = []
        for b in range(batch_size):
            flat = level_tensor[b].squeeze().cpu().numpy().flatten()
            inv = PCA_model.inverse_transform(flat).reshape(28, 28)
            inv = torch.clamp(torch.tensor(inv), 0.0, 1.0)
            imgs.append(inv)
        images_per_level.append(torch.stack(imgs))

    # plotting: rows=batch_size, cols=L
    fig, axs = plt.subplots(batch_size, L, figsize=(L * 1.5, batch_size * 1.5), squeeze=False)

    for b in range(batch_size):
        for l in range(L):
            img = images_per_level[l][b].numpy()
            ax = axs[b, l]
