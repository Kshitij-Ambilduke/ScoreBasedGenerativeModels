...
class CondInstanceNorm2d_plus(nn.Module):
    def __init__(self, L, C, labels=None):
        super().__init__()
        self.C = C
        ...
        if labels:
            self.label_embeddings = nn.Embedding(len(labels), C)
    
    def forward(self, x, sigma_index, label=None):
        """
        x : B x C x H x W
        sigma_index : B
        """
        ...
        if label is not None:
            label_emb = self.label_embeddings(label)  # B x C
            label_emb = label_emb.view(-1, self.C, 1, 1)  # B x C x 1 x 1
            x = x + label_emb
...
class CondChainedResidualPooling(nn.Module):
    def __init__(self, num_blocks=1, num_layers_per_block=2, in_channels=64, L=10, num_labels=None):
        super().__init__()
        ...
        for i in range(num_blocks):
            ...
            for j in range(num_layers_per_block):
                blocks.append(CondInstanceNorm2d_plus(L, in_channels, num_labels))
    ...
    def forward(self, x, sigma_index, label=None):
        ...
        for i in range(self.num_blocks):
            for j,layer in enumerate(self.CRP_blocks[i]):
                if label is not None:
                    path = self.norm_layers[i][j](path, sigma_index, label)
                else:
...
class CondResidualConvUnit(nn.Module):
    def __init__(self, num_blocks, num_layers_per_block, in_channels, L, num_labels=None):
        super().__init__()
        ...
        for i in range(num_blocks):
            blocks = nn.ModuleList()
            for j in range(num_layers_per_block):
                blocks.append(CondInstanceNorm2d_plus(L=L, C=in_channels, labels=num_labels))
    ...
    def forward(self, x, sigma_index, label=None):
        for i in range(self.num_blocks):
            ...
            for j,layer in enumerate(self.RCU[i]):
                if label is not None:
                    x = self.norms_layers[i][j](x, sigma_index, label)
                else:
...
class CondMultiResFusion(nn.Module):
    def __init__(self, in_planes, output_C, L=10, num_labels=None):
        super().__init__()
        ...
        for i in range(len(in_planes)):
            ...
            self.norm_layers.append(CondInstanceNorm2d_plus(L=L, C=in_planes[i], labels=num_labels))
                                    
    def forward(self, x_s, sigma_index, output_size, label=None):
        sum_x = torch.zeros(x_s[0].shape[0], self.output_C, *output_size, device=x_s[0].device)
        for i in range(len(self.in_planes)):
            if label is not None:
                h = self.norm_layers[i](x_s[i], sigma_index, label)
            else:
...
class CondRefineNet(nn.Module):
    ...
    def forward(self, x_s, sigma_index, output_shape, label=None):
        h_s = []
        
        # Adaptive Convolutional layer
        for i in range(len(x_s)):
            h = self.AdaptiveConv[i](x_s[i], sigma_index, label)
            h_s.append(h)
        ...
        if len(self.in_planes)==1:
            ...
            h = self.MRF(h_s, sigma_index, output_shape, label)

        # Chained residual pooling
        h = self.CRP(h, sigma_index, label)

        # Output Convolution
        h = self.OutputConv(h, sigma_index, label)
...
class ConditionalResidualBlock(nn.Module):
    def __init__(self, input_dim, output_dim, num_classes, resample=None, act=nn.ELU(),
                 normalization=CondInstanceNorm2d_plus, adjust_padding=False, dilation=None, labels=None):
        super().__init__()
        ...
        if resample == 'down':
            if dilation is not None:
                ...
                self.normalize2 = normalization(C=input_dim, L=num_classes, labels=labels)
            ...
            if dilation is not None:
                ...
                self.normalize2 = normalization(C=output_dim, L=num_classes, labels=labels)
        ...
        self.normalize1 = normalization(C=input_dim, L=num_classes, labels=labels)

    def forward(self, x, y, labels=None):
        output = self.normalize1(x, y, labels)
        output = self.non_linearity(output)
        output = self.conv1(output)
        output = self.normalize2(output, y, labels)
        output = self.non_linearity(output)
...
class CondRefineNetDilated(nn.Module):
    def __init__(self,
        ...
        self.normalizer = self.norm(C=ngf, L=self.num_classes, labels=labels)
        ...
        self.res1 = nn.ModuleList([
            ConditionalResidualBlock(self.ngf, self.ngf, self.num_classes, resample=None, act=act,
                                     normalization=self.norm, labels=labels),
            ConditionalResidualBlock(self.ngf, self.ngf, self.num_classes, resample=None, act=act,
                                     normalization=self.norm, labels=labels)]
        )
        ...
        self.res3 = nn.ModuleList([
            ConditionalResidualBlock(2 * self.ngf, 2 * self.ngf, self.num_classes, resample='down', act=act,
                                     normalization=self.norm, dilation=2, labels=labels),
            ConditionalResidualBlock(2 * self.ngf, 2 * self.ngf, self.num_classes, resample=None, act=act,
                                     normalization=self.norm, dilation=2, labels=labels)]
        )
        ...
        self.res4 = nn.ModuleList([
            ConditionalResidualBlock(2 * self.ngf, 2 * self.ngf, self.num_classes, resample='down', act=act,
                                        normalization=self.norm, adjust_padding=True, dilation=4, labels=labels),
            ConditionalResidualBlock(2 * self.ngf, 2 * self.ngf, self.num_classes, resample=None, act=act,
                                        normalization=self.norm, dilation=4, labels=labels)]
        )
    ...
    def _compute_cond_module(self, module, x, y, labels=None):
        for m in module:
            x = m(x, y, labels)
        return x
    ...
    def forward(self, x, y, labels=None):
        ...
        layer1 = self._compute_cond_module(self.res1, output, y, labels) # resnet layer 1 o/p = B x ngf x H x W = layer1
        layer2 = self._compute_cond_module(self.res2, layer1, y, labels) # resnet layer 2 o/p = B x 2ngf x H/2 x W/2 = layer2
        layer3 = self._compute_cond_module(self.res3, layer2, y, labels) # resnet layer 3 o/p = B x 2ngf x H/4 x W/4 = layer3
        layer4 = self._compute_cond_module(self.res4, layer3, y, labels) # resnet layer 4 o/p = B x 2ngf x H/8 x W/8 = layer4

        ref1 = self.refine1([layer4], y, layer4.shape[2:], labels) #RefineNet 1: input: layer4 (B x 2ngf x H/8 x W/8) output: ref1 (B x 2ngf x H/8 x W/8)
        ref2 = self.refine2([layer3, ref1], y, layer3.shape[2:], labels) #RefineNet 2: input: layer3 (B x 2ngf x H/4 x W/4), ref1 (B x 2ngf x H/8 x W/8) output: ref2 (B x 2ngf x H/4 x W/4)

        ref3 = self.refine3([layer2, ref2], y, layer2.shape[2:], labels) #RefineNet 3: input: layer2 (B x 2ngf x H/2 x W/2), ref2 (B x 2ngf x H/4 x W/4) output: ref3 (B x ngf x H/2 x W/2)
        output = self.refine4([layer1, ref3], y, layer1.shape[2:], labels) #RefineNet 4: input: layer1 (B x ngf x H x W), ref3 (B x ngf x H/2 x W/2) output: output (B x ngf x H x W)

        output = self.normalizer(output, y, labels)
...
# labels = torch.randint(0,5,(4,)) # batch_size=4, num_labels=5
# # print("Y:", y.shape)

# model = CondRefineNetDilated(input_channels=1, L=10, ngf=64, labels=[0,1,2,3,4])
# out = model(x,y,labels)
