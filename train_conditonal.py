...
def load_data(allowed_classes=[0,1,2], split="train", sigma_1=1.0, sigma_L=0.01, L=10, batch_size=128):
    dataset = ScoreGenerationDataset(dataset_name="fmnist", 
                                     ...
                                     PCA_components=64,
                                     PCA_model_save_path="pca_fmnist_64_components.joblib")
...
def main():
    ...
    while steps<total_steps:
        ...
        for batch in tqdm(dataloader, total=len(dataloader)):
            ...
            optimizer.zero_grad()
            outputs = model(noisy, sigma_index, labels)
        ...
        epoch_steps += 1
        torch.save(model.state_dict(), f"SAVED_MODEL_{epoch_steps}_fmnist_conditional.pt")
